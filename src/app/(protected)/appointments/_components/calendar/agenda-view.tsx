  ...
  return (
    <div className="flex h-full flex-col overflow-hidden p-1 text-white">
      <div className="flex items-center gap-2">
        ...
        {/* Badge de status removido pois 'status' não existe no tipo centralizado */}
      </div>
...
export default function AgendaView({ appointments }: AgendaViewProps) {
  ...
  const { execute: executeUpdate } = useAction(updateAppointmentDate, {
    ...
    onError: (err) => {
      setLoading(false);
      toast.error(err.error?.serverError || "Erro ao reagendar.");
    },
  });

  const handleEventDrop = useCallback(
    (args: { event: RBCEvent; start: Date; end: Date }) => {
      setLoading(true);
